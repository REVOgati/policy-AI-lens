...
async def extract_text_from_pdf(file_path: str) -> str:
    ...
    try:
        ...
        if len(extracted_text.strip()) > 1000:
            ...
            print(f"⚠️  PyMuPDF extracted only {len(extracted_text)} characters, trying OCR...")
    ...
    try:
        ...
        poppler_path = None
        base_paths = [
            r"C:\Program Files\poppler\poppler-24.08.0\Library\bin",  # Manual install (specific version)
        ...
        for base in base_paths:
            if os.path.exists(base):
                # If it's already a bin directory with pdftoppm.exe, use it directly
        ...
        print(f"Extracted {len(extracted_text)} characters using Tesseract OCR")
        return extracted_text
