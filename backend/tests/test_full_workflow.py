...
PDF_PATH = input(
    "Enter path to your insurance policy PDF (or press Enter to skip): ").strip()
...
try:
    ...
    new_policy = input(
        "Enter corrected policy number (or press Enter to keep): ").strip()
    if new_policy:
    ...
    print(
        f"   Edited: {', '.join(edited_fields) if edited_fields else 'None'}")
    ...
    print(
        f"   2. Extracted: {len([v for v in verified_data.values() if v])} fields found")
